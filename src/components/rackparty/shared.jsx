...
export function loadGoogleFont(fontUrl) {
  if (!fontUrl) return;
...
export function toggleFavorite(templateId, accentColor) {
...
export function isFavorited(templateId, accentColor) {
...
export function Step3_Location({ venueName, city, address, onChangeVenue, onChangeCity, onChangeAddress, onNext, onBack }) {
              ...
              <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {EXPORT_SIZES.map(size => (
                    <Button
                      key={size.label} variant="outline" size="sm"
                      onClick={() => { setExportSize(size); setTimeout(() => handleExportSize(size), 100); }}
                      disabled={generatingImage} className="rounded-lg gap-1.5 text-xs"
                    >
