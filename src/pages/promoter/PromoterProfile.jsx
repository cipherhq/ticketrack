...
export function PromoterProfile() {
  ...
  if (!promoter) return <Card className="border-[#0F0F0F]/10 rounded-2xl"><CardContent className="p-8 text-center"><UserCircle className="w-16 h-16 text-[#0F0F0F]/20 mx-auto mb-4" /><h3 className="text-lg">Profile Not Found</h3></CardContent></Card>;

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h2 className="text-2xl text-[#0F0F0F] mb-2">Promoter Profile</h2><p className="text-[#0F0F0F]/60">Manage your information</p></div>
        ...
        <CardContent className="p-6">
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2"><h2 className="text-2xl text-[#0F0F0F]">{promoter.full_name}</h2><Badge className={promoter.status === 'active' ? 'bg-green-600' : 'bg-yellow-600'}>{promoter.status}</Badge></div>
              <p className="text-[#0F0F0F]/60 mb-3">{promoter.bio || 'No bio added yet.'}</p>
              <div className="grid md:grid-cols-2 gap-4">
                <div className="flex items-center gap-2 text-sm text-[#0F0F0F]/60"><Mail className="w-4 h-4" /><span>{promoter.email}</span></div>
                <div className="flex items-center gap-2 text-sm text-[#0F0F0F]/60"><Phone className="w-4 h-4" /><span>{promoter.phone || 'Not provided'}</span></div>
                <div className="flex items-center gap-2 text-sm text-[#0F0F0F]/60"><Calendar className="w-4 h-4" /><span>Joined {new Date(promoter.created_at).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</span></div>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <Card className="border-[#0F0F0F]/10 rounded-2xl">
        ...
        <CardContent>
          {isEditing ? (
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="p-4 bg-[#F4F6FA] rounded-xl"><p className="text-sm text-[#0F0F0F]/60 mb-1">Full Name</p><p>{promoter.full_name}</p></div>
              <div className="p-4 bg-[#F4F6FA] rounded-xl"><p className="text-sm text-[#0F0F0F]/60 mb-1">Email</p><p>{promoter.email}</p></div>
              <div className="p-4 bg-[#F4F6FA] rounded-xl"><p className="text-sm text-[#0F0F0F]/60 mb-1">Phone</p><p>{promoter.phone || 'Not provided'}</p></div>
              <div className="p-4 bg-[#F4F6FA] rounded-xl"><p className="text-sm text-[#0F0F0F]/60 mb-1">Status</p><p className="capitalize">{promoter.status}</p></div>
              <div className="p-4 bg-[#F4F6FA] rounded-xl md:col-span-2"><p className="text-sm text-[#0F0F0F]/60 mb-1">Bio</p><p>{promoter.bio || 'No bio added yet.'}</p></div>
            </div>
          )}
        </CardContent>
      </Card>
