...
import { CreditCard, Building2, Smartphone, Lock, ArrowLeft, Loader2, Calendar, MapPin } from 'lucide-react'
...
export function WebCheckout() {
  ...
  const ticketSummary = Object.entries(selectedTickets)
    ...
    .map(([tierId, qty]) => {
      ...
      return { name: tier?.name || 'Ticket', quantity: qty, price: tier?.price || 0, subtotal: (tier?.price || 0) * qty }
    })

  const totalTicketCount = ticketSummary.reduce((sum, t) => sum + t.quantity, 0)

  // Function to finalize order after payment
  const finalizeOrder = async (orderId, reference) => {
    try {
      // Update order status
      ...
      for (const item of ticketSummary) {
        const ticketType = ticketTypes.find(t => t.name === item.name)
        for (let i = 0; i < item.quantity; i++) {
          ticketsToCreate.push({
            ...
            qr_code: `TKT${Date.now().toString(36).toUpperCase()}${i}`,
            status: 'valid'
          })
        }
      }
      ...
      return { success: true, tickets: ticketsToCreate }
    } catch (err) {
  ...
  const handlePaystackPayment = async () => {
    ...
    try {
      ...
      const orderItems = ticketSummary.map(ticket => ({
        order_id: order.id,
        ticket_type_id: ticketTypes.find(t => t.name === ticket.name)?.id,
      ...
      if (window.PaystackPop) {
        const handler = window.PaystackPop.setup({
          ...
          callback: function(response) {
            ...
            finalizeOrder(order.id, response.reference).then(result => {
              if (result.success) {
        ...
        <div className="lg:col-span-1">
          <Card className="border-[#0F0F0F]/10 rounded-2xl sticky top-24">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-3">
                {ticketSummary.map((ticket, index) => (
                  <div key={index} className="flex justify-between text-sm">
                    ...
                    <span className="text-[#0F0F0F]">₦{ticket.subtotal.toLocaleString()}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm"><span className="text-[#0F0F0F]/60">Subtotal ({totalTicketCount} tickets)</span><span className="text-[#0F0F0F]">₦{totalAmount.toLocaleString()}</span></div>
                <div className="flex justify-between text-sm"><span className="text-[#0F0F0F]/60">Service Fee</span><span className="text-[#0F0F0F]">₦{serviceFee.toLocaleString()}</span></div>
                <Separator />
                <div className="flex justify-between font-bold text-lg"><span className="text-[#0F0F0F]">Total</span><span className="text-[#2969FF]">₦{finalTotal.toLocaleString()}</span></div>
              </div>

              <Button className="w-full bg-[#2969FF] hover:bg-[#1a4fd8] text-white rounded-xl py-6 text-lg" onClick={handlePaystackPayment} disabled={loading || !formData.email || !formData.firstName || !formData.lastName}>
                {loading ? (<><Loader2 className="w-5 h-5 mr-2 animate-spin" />Processing...</>) : (<><Lock className="w-5 h-5 mr-2" />Pay ₦{finalTotal.toLocaleString()}</>)}
              </Button>
