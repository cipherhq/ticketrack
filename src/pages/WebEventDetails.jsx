          ...
          <div className="relative">
            ...
            <div className="absolute top-0 right-0 z-10 flex gap-2 sm:relative sm:z-auto">
              <Button 
                ...
                className={`rounded-xl w-11 h-11 touch-manipulation flex-shrink-0 flex items-center justify-center ${isFavorite ? 'text-red-500 border-red-500 bg-red-50 hover:bg-red-100' : ''}`}
                aria-label={isFavorite ? 'Remove from favorites' : 'Add to favorites'}
              >
                {savingFavorite ? (
                  <Loader2 className="w-5 h-5 min-w-5 min-h-5 animate-spin" />
                ) : (
                  <Heart className={`w-5 h-5 min-w-5 min-h-5 ${isFavorite ? 'fill-red-500' : ''}`} />
                )}
              </Button>
              <Button 
                variant="outline" 
                size="icon" 
                className="rounded-xl w-11 h-11 touch-manipulation flex-shrink-0 flex items-center justify-center"
                onClick={(e) => { e.stopPropagation(); handleShare(); }}
                aria-label="Share event"
              >
                <Share2 className="w-5 h-5 min-w-5 min-h-5" />
              </Button>
            </div>
