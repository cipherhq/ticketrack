        ...
        (followersData || []).map(async (follower) => {
          ...
          if (eventIds.length > 0) {
            const { data: tickets } = await supabase
              .from('tickets')
              .select('id, total_amount, created_at, checked_in_at')
              .eq('user_id', follower.user_id)
              .in('event_id', eventIds)
              .eq('payment_status', 'completed');

            eventsAttended = tickets?.filter(t => t.checked_in_at)?.length || 0;
            totalSpent = tickets?.reduce((sum, t) => sum + (t.total_amount || 0), 0) || 0;
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-4">
                  ...
                  <div className="grid grid-cols-3 gap-4 text-center">
                    ...
                    <div className="p-2 bg-[#F4F6FA] rounded-lg">
                      ...
                      <p className="text-lg font-semibold text-[#0F0F0F]">{formatCurrency(follower.totalSpent)}</p>
                    </div>
                    <div className="p-2 bg-[#F4F6FA] rounded-lg">
                      <p className="text-xs text-[#0F0F0F]/60">Last Active</p>
                      <p className="text-sm font-medium text-[#0F0F0F]">
                        {new Date(follower.lastActivity).toLocaleDateString('en-NG', { month: 'short', day: 'numeric' })}
                      </p>
                    </div>
                  </div>
                </div>
