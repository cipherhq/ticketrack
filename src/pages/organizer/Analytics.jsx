...
export function Analytics() {
  ...
  const loadStats = async () => {
    ...
    const grossRevenue = tickets?.reduce((sum, t) => sum + (parseFloat(t.total_price) || 0), 0) || 0;
    ...
    const averageTicketPrice = totalTickets > 0 ? grossRevenue / totalTickets : 0;
  ...
  const loadMonthlyRevenue = async () => {
    ...
    tickets?.forEach(ticket => {
      ...
      if (monthIndex !== -1) {
        ...
        months[monthIndex].platformFee += amount * 0.10;
        months[monthIndex].net += amount * 0.90;
      }
    });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="border-[#0F0F0F]/10 rounded-2xl">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-[#0F0F0F]/60">Gross Revenue</span>
              <DollarSign className="w-5 h-5 text-[#2969FF]" />
            </div>
            <p className="text-2xl font-semibold text-[#0F0F0F]">{formatCurrency(stats.grossRevenue)}</p>
            <p className="text-xs text-[#0F0F0F]/40 mt-1">Total ticket sales</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-[#0F0F0F]/10 rounded-2xl">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-[#0F0F0F]/60">Net Revenue</span>
              <DollarSign className="w-5 h-5 text-green-600" />
            </div>
            <p className="text-2xl font-semibold text-green-600">{formatCurrency(stats.netRevenue)}</p>
            <p className="text-xs text-[#0F0F0F]/40 mt-1">After 10% platform fee</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-[#0F0F0F]/10 rounded-2xl">
          <CardContent className="p-6">
            ...
            <p className="text-xs text-[#0F0F0F]/40 mt-1">Avg: {formatCurrency(stats.averageTicketPrice)}</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-[#0F0F0F]/10 rounded-2xl">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-[#0F0F0F]/60">Followers</span>
              <Users className="w-5 h-5 text-[#2969FF]" />
            </div>
            <p className="text-2xl font-semibold text-[#0F0F0F]">{followerStats.total.toLocaleString()}</p>
            <div className="flex items-center gap-1 mt-1">
              {followerStats.growth > 0 ? (
                <TrendingUp className="w-3 h-3 text-green-600" />
              ) : (
                <TrendingDown className="w-3 h-3 text-red-600" />
              )}
              <span className={`text-xs ${followerStats.growth > 0 ? 'text-green-600' : 'text-red-600'}`}>
                {followerStats.growth.toFixed(1)}% this month
              </span>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-[#0F0F0F]">{formatCurrency(stats.platformFees)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
