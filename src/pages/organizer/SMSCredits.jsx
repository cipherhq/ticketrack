    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#0F0F0F]/60 mt-1">Buy credits to send SMS to your attendees</p>
        </div>
      ...
      <Card className="border-[#0F0F0F]/10 rounded-2xl bg-gradient-to-r from-[#2969FF] to-[#1e4fcc]">
      ...
      <Tabs defaultValue="packages">
          ...
          {packages.length === 0 ? (
            <Card className="border-[#0F0F0F]/10 rounded-2xl">
              <CardContent className="p-8 text-center">
                <p className="text-[#0F0F0F]/60">No packages available.</p>
              </CardContent>
            </Card>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {packages.map((pkg) => (
                <Card key={pkg.id} className={`border-2 rounded-2xl relative ${pkg.is_popular ? 'border-[#2969FF]' : 'border-[#0F0F0F]/10'}`}>
                  {pkg.is_popular && (
                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="mt-4">
                      <p className="text-3xl font-bold text-[#0F0F0F]">{pkg.credits}<span className="text-base font-normal text-[#0F0F0F]/60 ml-1">credits</span></p>
                      {pkg.bonus_credits > 0 && <p className="text-green-600 text-sm mt-1">+{pkg.bonus_credits} bonus!</p>}
                    </div>
                    <div className="mt-4 p-3 bg-[#F4F6FA] rounded-xl">
                      <div className="flex justify-between"><span className="text-[#0F0F0F]/60">Price</span><span className="font-semibold">{formatCurrency(pkg.price)}</span></div>
                      <div className="flex justify-between mt-1"><span className="text-[#0F0F0F]/60">Per SMS</span><span>₦{parseFloat(pkg.price_per_sms || (pkg.price / pkg.credits)).toFixed(2)}</span></div>
                    </div>
            ...
            <CardContent>
              {purchases.length === 0 ? (
                ...
                <div className="space-y-3">
                  {purchases.map((p) => (
                    <div key={p.id} className="flex justify-between p-4 bg-[#F4F6FA] rounded-xl">
                      <div>
                        <p className="font-medium">{p.sms_credit_packages?.name || 'Package'}</p>
                        <p className="text-sm text-[#0F0F0F]/60">{p.credits_purchased}{p.bonus_credits > 0 && ` + ${p.bonus_credits} bonus`}</p>
                        <p className="text-xs text-[#0F0F0F]/40 mt-1">{new Date(p.created_at).toLocaleString()}</p>
                      </div>
        ...
        <TabsContent value="usage" className="mt-6">
          <Card className="border-[#0F0F0F]/10 rounded-2xl">
            ...
            <CardContent>
              {usage.length === 0 ? (
                <p className="text-center text-[#0F0F0F]/60 py-8">No usage yet</p>
              ) : (
                <div className="space-y-3">
                  {usage.map((u) => (
                    <div key={u.id} className="flex justify-between p-4 bg-[#F4F6FA] rounded-xl">
                      <div>
                        <p className="font-medium">{u.events?.title || 'Campaign'}</p>
                        <p className="text-sm text-[#0F0F0F]/60">{u.recipient_count} recipients</p>
                        <p className="text-xs text-[#0F0F0F]/40 mt-1">{new Date(u.created_at).toLocaleString()}</p>
                      </div>
                      <div className="text-right">
                        <p className="font-semibold text-red-600">-{u.credits_used}</p>
                        <p className="text-xs text-[#0F0F0F]/60">credits</p>
                      </div>
                    </div>
                  ))}
                </div>
