...
const categories = [
  ...
  { icon: Settings, title: 'Technical Support', description: 'App issues, troubleshooting, and technical help', color: 'bg-background0' },
]
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="text-center mb-12">
        ...
        <div className="max-w-2xl mx-auto relative">
          ...
          <Input placeholder="Search for help..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-12 h-14 rounded-2xl border-border/10 text-base" />
        </div>
      </div>
        ...
        <div className="mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {categories.map((category, index) => {
              ...
              return (
                <Card key={index} className="border-border/10 rounded-2xl hover:border-[#2969FF]/30 transition-colors cursor-pointer">
      ...
      <div>
        <h2 className="text-2xl font-bold text-foreground mb-6">{searchQuery ? 'Search Results' : 'Frequently Asked Questions'}</h2>
          ...
          <div className="space-y-8">
            {filteredFaqs.map((category, categoryIndex) => (
              <div key={categoryIndex}>
                {!searchQuery && <h3 className="text-xl font-semibold text-foreground mb-4">{category.category}</h3>}
                <Accordion type="single" collapsible className="space-y-4">
                  {category.questions.map((faq, faqIndex) => (
                    <AccordionItem key={faqIndex} value={`${categoryIndex}-${faqIndex}`} className="border border-border/10 rounded-2xl px-6 bg-card">
                      <AccordionTrigger className="text-foreground hover:no-underline py-6">{faq.question}</AccordionTrigger>
                      <AccordionContent className="text-foreground/70 pb-6">{faq.answer}</AccordionContent>
                    </AccordionItem>
                  ))}
                </Accordion>
              </div>
      ...
      <Card className="border-[#2969FF]/20 bg-[#2969FF]/5 rounded-2xl mt-16">
        <CardContent className="p-8 text-center">
          ...
          <p className="text-foreground/70 mb-6">Can't find what you're looking for? Our support team is here to help.</p>
