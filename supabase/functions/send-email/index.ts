...
const EMAIL_PERMISSIONS: Record<string, { auth: AuthLevel; rateKey: string; fromEmail?: string; allowAnon?: boolean; verifyOrder?: boolean }> = {
  ...
  waitlist_new_signup: { auth: 'SYSTEM_ONLY', rateKey: 'standard' },
  ...
  admin_payout_completed: { auth: 'SYSTEM_ONLY', rateKey: 'standard' },
  ...
  // Ad approval emails (admin-triggered)
  ad_approved: { auth: 'ADMIN_AUTH', rateKey: 'standard' },
  ad_rejected: { auth: 'ADMIN_AUTH', rateKey: 'standard' },
  // Admin auth emails
...
const templates: Record<string, (d: any) => { subject: string; html: string }> = {
  ...
  waitlist_new_signup: d => ({ subject: `📋 New waitlist signup — ${d.eventTitle}`, html: baseTemplate(`<h2>New Waitlist Signup</h2><p>Hi ${escapeHtml(d.organizerName)},</p><p>Someone just joined the waitlist for <strong>${escapeHtml(d.eventTitle)}</strong>.</p><div class="card"><div class="row"><span class="label">Name</span><span class="value">${escapeHtml(d.attendeeName)}</span></div><div class="row"><span class="label">Email</span><span class="value">${escapeHtml(d.attendeeEmail)}</span></div><div class="row"><span class="label">Tickets wanted</span><span class="value">${d.quantity}</span></div><div class="row"><span class="label">Position</span><span class="value" style="color:${BRAND_COLOR};font-weight:bold">#${d.position}</span></div><div class="row"><span class="label">Total on waitlist</span><span class="value" style="font-weight:bold">${d.totalWaitlist}</span></div></div><div class="btn-wrap"><a href="${APP_URL}/organizer/events" class="btn">Manage Events</a></div>`, `${escapeHtml(d.attendeeName)} joined the waitlist for ${escapeHtml(d.eventTitle)}`) }),
  // SOCIAL
...
<body style="margin: 0; padding: 0; background-color: #f4f4f7; font-family: Arial, sans-serif;">
          ...
          <tr>
            <td align="center" style="padding: 0 40px 40px 40px;">
              <table role="presentation" cellspacing="0" cellpadding="0" border="0">
                <tr>
                  <td align="center" style="background-color: #2969FF; border-radius: 8px;">
                    <a href="${APP_URL}/organizer/events" style="display: inline-block; padding: 16px 40px; font-size: 16px; font-weight: 600; color: #ffffff; text-decoration: none;">View Dashboard</a>
                  </td>
                </tr>
              </table>
            </td>
  ...
  low_ticket_alert: d => ({ subject: `⚠️ Low Tickets: ${d.eventTitle}`, html: baseTemplate(`<div class="highlight"><strong>Running Low!</strong></div><h2>${d.eventTitle}</h2><div class="card"><div class="row"><span class="label">${d.ticketType}</span><span class="value" style="color:#dc3545">${d.remaining} left</span></div><div class="row"><span class="label">Sold</span><span class="value">${d.sold}/${d.total}</span></div></div><div class="btn-wrap"><a href="${APP_URL}/organizer/events" class="btn">Manage</a></div>`) }),
...
<body style="margin: 0; padding: 0; background-color: #f4f4f7; font-family: Arial, sans-serif;">
          ...
          <tr>
            <td align="center" style="padding: 0 40px 40px 40px;">
              <table role="presentation" cellspacing="0" cellpadding="0" border="0">
                <tr>
                  <td align="center" style="background-color: #2969FF; border-radius: 8px;">
                    <a href="${APP_URL}/organizer/events" style="display: inline-block; padding: 16px 40px; font-size: 16px; font-weight: 600; color: #ffffff; text-decoration: none;">View Event Dashboard</a>
                  </td>
                </tr>
              </table>
            </td>
  ...
  event_published: d => ({ subject: `✅ Published: ${d.eventTitle}`, html: baseTemplate(`<div class="success"><strong>Live!</strong></div><h2>${d.eventTitle}</h2><p>Your event is visible.</p><div class="card"><div class="row"><span class="label">Date</span><span class="value">${formatDate(d.eventDate)}</span></div><div class="row"><span class="label">Link</span><span class="value"><a href="${d.eventUrl}">${d.eventUrl}</a></span></div></div><div class="btn-wrap"><a href="${APP_URL}/organizer/events" class="btn">Manage</a></div>`) }),
  ...
  post_event_summary: d => ({ subject: `Summary: ${d.eventTitle}`, html: baseTemplate(`<h2>Event Wrap-Up</h2><p><strong>${d.eventTitle}</strong></p><div class="card"><div class="row"><span class="label">Revenue</span><span class="value" style="color:${BRAND_COLOR};font-size:24px">${formatCurrency(d.totalRevenue, d.currency)}</span></div><div class="row"><span class="label">Tickets</span><span class="value">${d.ticketsSold}</span></div><div class="row"><span class="label">Checked In</span><span class="value">${d.checkedIn} (${d.checkInRate}%)</span></div><div class="row"><span class="label">Payout</span><span class="value">${formatCurrency(d.payoutAmount, d.currency)}</span></div></div><p>Payout in 2-3 business days.</p><div class="btn-wrap"><a href="${APP_URL}/organizer/events" class="btn">Full Report</a></div>`) }),
  ...
  admin_payout_completed: d => ({ subject: `💰 Payout Completed - ${d.organizerName}`, html: baseTemplate(`<div class="success"><strong>Payout Processed</strong></div><h2>Organizer Payout Complete</h2><div class="card"><div class="row"><span class="label">Organizer</span><span class="value">${d.organizerName}</span></div><div class="row"><span class="label">Email</span><span class="value">${d.organizerEmail}</span></div><div class="row"><span class="label">Gross Amount</span><span class="value">${formatCurrency(d.amount, d.currency)}</span></div><div class="row"><span class="label">Platform Fee</span><span class="value" style="color:#dc3545">-${formatCurrency(d.platformFee, d.currency)}</span></div><div class="row"><span class="label">Net Payout</span><span class="value" style="color:#10b981;font-size:20px">${formatCurrency(d.netAmount, d.currency)}</span></div><div class="row"><span class="label">Bank</span><span class="value">${d.bankName}</span></div><div class="row"><span class="label">Account</span><span class="value" style="font-family:monospace">${d.accountNumber}</span></div><div class="row"><span class="label">Reference</span><span class="value" style="font-family:monospace">${d.reference}</span></div><div class="row"><span class="label">Processed By</span><span class="value">${d.processedBy || 'Admin'}</span></div><div class="row"><span class="label">Time</span><span class="value">${formatDate(d.processedAt)}</span></div></div><div class="btn-wrap"><a href="${APP_URL}/admin/payouts" class="btn">View Payouts</a></div>`) }),
  // PAYSTACK PAYOUTS
